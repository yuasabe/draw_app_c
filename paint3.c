...
// ファイルからコマンド履歴をロードし、描写
// 最新のHistoryリストの先頭を返す
History *load(const char *command, History *h, Canvas *c) {
  char buf[bufsize];
  strcpy(buf, command);
  buf[strlen(buf) - 1] = 0;
  char *s = strtok(buf, " ");
  ...
  if (strcmp(s, "load") == 0) {
    ...
    while (fgets(command, 31, fp) != NULL) {
      h = push_history(h, command);
      printf("%s",command);
      interpret_command(command, NULL, c);
    }
    fclose(fp);
    return h;
  }
...
int interpret_command(const char *command, History *h, Canvas *c) {
  ...
  if (strcmp(s, "pen") == 0) {
    char *pen_type = strtok(NULL, " ");
    if (pen_type == NULL) {
      c->pen = '#';
    } else {
      c->pen = pen_type[strlen(pen_type)-1];
    }
